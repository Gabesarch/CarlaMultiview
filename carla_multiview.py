...
class CarlaMultiviewRunner():
    def __init__(self):
        ...
        self.base_path = "_carla_multiview"
        self.host = "127.0.0.1"
        self.port = 2000
        ...
        self.num_datapoints_per_episode = 20
        self.min_frame_difference = 50

        self.should_randomize_camR = True
        self.camR_sensor_indices = []
        if self.should_randomize_camR:
            self.camR_Xrange = [4.5, 14]
            self.camR_Yrange = [-5, 5]
    ...
    def calculate_camera_locations(self):
        ...
        self.positions.append([4.5, 0.0, 1.0]) #CamR
        self.rotations.append([-180.0, 0, 180])  #CamR

        self.positions.append([-4.5, 2.0, 2.5])
        self.rotations.append([8.0, -20, 0])

        self.positions.append([5.0, 0.5, 1.5]) #check y val
        self.rotations.append([0, 180, 0])

        self.positions.append([-3.0, 0, 4.0])
        self.rotations.append([-40, 0, 0])

        self.positions.append([-0.5, 3.0, 4.0])
        self.rotations.append([-36.0, -56.0, 0])

        self.positions.append([1.5, -4.0, 4.0])
        self.rotations.append([-26.0, -250.0, 0])

        self.positions.append([0.5, -2, 4.5])
        self.rotations.append([-90.0, 0, 0])




        # self.positions = [[-3, 0, 3], [-7, 0, 3]]
        # self.rotations = [[0, 0, 0], [0, 0, 0]]
        # self.positions = [[5, 0, 0]]
    ...
    def spawn_vehicles(self, blueprints, episode_path, spawn_points):
        ...
        for n, transform in enumerate(spawn_points):
            ...
            vehicle_path = os.path.join(episode_path, str(vehicle.id))

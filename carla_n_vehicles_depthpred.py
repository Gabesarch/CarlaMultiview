...
class CarlaMultiviewRunner():
    def __init__(self, blueprint_num):
        ...
        self.base_path = "_carla_1_vehicles_depth_B"
        ...
        self.min_vehicles = 1
        self.max_vehicles = 3
        self.stereo_baseline = 0.25
        self.maximum_spawn_distance = 10
        ...
        self.image_size_x = 330
        self.image_size_y = 330
        self.fov = 110
    ...
    def spawn_vehicles(self, blueprints, episode_path, spawn_points):
        ...
        for j in range(1, number_of_spawn_points):
            if self.pts_dist(locs[0], locs[j]) < allowed_dist:
                mindist = self.pts_dist(locs[0], locs[j])
                allowed_second_spawn.append(spawn_points[j])
        ...
        selected_transforms = []
        for spawn_i in range(num_cars_to_spawn):
            closest_transform = allowed_second_spawn[spawn_i]
            selected_transforms.append(closest_transform)
    ...
    def get_mean_position_rotation(self, spawn1, spawn2):
        ...
        # else:
        #     radius = 6
        # self.positions.append([radius, -ydiff, 5 + np.random.uniform(-0.5,0.5)])
        # self.rotations.append([0, -180, 0])

        self.num_camRs = 0 # Number of camR candidates
        ...
        for yaw in range(0, 360, 15):
            ...
            if self.randomize_each_camera:
                radius = radius_orig + np.random.uniform(-0.5,0.5)
            else:
                radius = radius_orig
            self.num_camRs += 1
            yaw_rads = np.radians(yaw)
            zcoord = 5 + np.random.uniform(-0.5,0.5)
            ...
            #Add stereo camera
            self.num_camRs += 1
            radius_stereo = np.sqrt(radius**2 + self.stereo_baseline**2)
            yaw_rads_stereo = yaw_rads + np.arcsin(self.stereo_baseline*1./radius_stereo)
            xcoord = -radius_stereo*np.cos(yaw_rads_stereo) - xdiff
            ycoord = radius_stereo*np.sin(yaw_rads_stereo) - ydiff
            print("Stereo camera params are: ", xcoord, ycoord, zcoord, yaw)
            self.positions.append([xcoord, ycoord, zcoord])
            self.rotations.append([-0, -yaw, 0])
            if count >= 10:
                break
        ...
        for yaw in range(0, 360, 15):
            ...
            if self.randomize_each_camera:
                ...
                radius = radius_orig
            yaw_rads = np.radians(yaw)
            zcoord = 5.0 + np.random.uniform(-0.5,0.5)
            xcoord = -radius*np.cos(yaw_rads) - xdiff
            ycoord = radius*np.sin(yaw_rads) - ydiff
            print("camera params are: ", xcoord, ycoord, zcoord, yaw)
            self.positions.append([xcoord, ycoord, zcoord])
